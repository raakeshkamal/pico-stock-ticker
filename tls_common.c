...
static err_t tls_client_recv(void *arg, struct altcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (p->tot_len > 0) {
        if (state->recv_buffer && state->recv_buffer_size > 0) {
            size_t copy_len = p->tot_len > state->recv_buffer_size ? state->recv_buffer_size : p->tot_len;
...
static void tls_client_connect_to_server_ip(const ip_addr_t *ipaddr, TLS_CLIENT_T *state, uint16_t port)
{
    err_t err;
...
static void tls_client_dns_found(const char* hostname, const ip_addr_t *ipaddr, void *arg)
{
    ...
    if (ipaddr) {
        ...
        tls_client_connect_to_server_ip(ipaddr, state, 8443); // TODO: Make port configurable
    } else {
...
static bool tls_client_open(const char *hostname, TLS_CLIENT_T *state) {
    ...
    if (err == ERR_OK) {
        tls_client_connect_to_server_ip(&server_ip, state, 8443); // TODO: Make port configurable
...
TLS_CLIENT_HANDLE tls_client_init_and_connect(const char *server_hostname, uint16_t server_port,
    ...
    if (!state->complete_sem || !state->recv_sem) {
        ...
        if (state->complete_sem) vSemaphoreDelete(state->complete_sem);
        if (state->recv_sem) vSemaphoreDelete(state->recv_sem);
        free(state);
        return NULL;
    }
...
int tls_client_send_and_recv(TLS_CLIENT_HANDLE handle,
    ...
    err_t err = altcp_write(state->pcb, send_buffer, send_len, TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) {
